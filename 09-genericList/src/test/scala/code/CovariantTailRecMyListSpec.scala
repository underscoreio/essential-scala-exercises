...
class CovariantTailRecMyListSpec extends FlatSpec with Matchers {

  // Helper method to make it easier to build lists.
  // You'll see how this works in the Sequencing Computations section:
  def list[A](items: A *): CovariantTailRecMyList[A] =
    items.foldRight[CovariantTailRecMyList[A]](CovariantTailRecMyNil)(CovariantTailRecMyPair[A])
