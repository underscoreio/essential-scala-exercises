...
class CovariantTreeSpec extends FlatSpec with Matchers {
  ...
  def leaf[A]: CovariantTree[A] =
    CovariantLeaf()

  def node[A](n: A, l: CovariantTree[A] = leaf[A], r: CovariantTree[A] = leaf[A]): CovariantTree[A] =
    CovariantNode(n, l, r)
