...
object FilmTestData {
  ...
  val eastwood: Director =
    Director("Clint", "Eastwood", 1930, List(
      highPlainsDrifter,
      outlawJoseyWales,
      unforgiven,
      granTorino,
  ...
  val mcTiernan: Director =
    Director("John", "McTiernan", 1951, List(
      predator,
      dieHard,
      huntForRedOctober,
      thomasCrownAffair,
  ...
  val nolan: Director =
    Director("Christopher", "Nolan", 1970, List(
      memento,
      darkKnight,
      inception,
    ))
  ...
  val someGuy: Director =
    Director("Just", "Some Guy", 1990, Nil)

  val directors: List[Director] =
    List(someGuy, mcTiernan, nolan, eastwood)
